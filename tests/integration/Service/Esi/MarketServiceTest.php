...
JSON;
        ...
        /** @var CharactersOrdersHistoryResponseV1 $order */
        $order = $result->first();
...
JSON;
        ...
        /** @var MarketServiceInterface $service */
        $service = $this->tester->grabService('test.f1monkey.eve_esi.market_service');
        $result  = $service->getV2CharactersOrders('code', $characterId);

        static::assertSame(
            'https://esi.evetech.net/v2/characters/10/orders/',
            $this->httpMock->getLastRequestUrl()
        );

        /** @var CharactersOrdersResponseV2 $order */
        $order = $result->first();
